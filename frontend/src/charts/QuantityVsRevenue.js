...
const QuantityVsRevenue = ({ scatter }) => {
  const data = useMemo(() => ({
    datasets: [
      {
        label: 'Producto',
        data: scatter.map((r) => ({ x: r.cantidad, y: r.ingreso_sin_iva, label: r.Producto })),
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: ({ raw }) =>
            [`Cantidad: ${raw.x.toLocaleString('es-CL')} uds.`, `Ingreso: ${CLP(raw.y)}`],
        },
      },
    },
    scales: {
      x: {
        ...
        beginAtZero: true,
        title: { display: true, text: 'Unidades vendidas', color: COLORS.text, font: { size: 11 } },
        ticks: {
  ...
  return (
    <Box>
      ...
      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>
        Top 40 productos por ingresos · pasa el cursor sobre un punto para ver el producto
      </Typography>
      <Box sx={{ height: 320 }}>
