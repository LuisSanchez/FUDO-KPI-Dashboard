...
import {
  ...
  CircularProgress, Chip, Divider, Alert, InputAdornment,
} from '@mui/material';
...
// ── TargetCard ────────────────────────────────────────────────────────────────
const TargetCard = ({
  icon, title, alreadyMet, impossible,
  totalUnits, currentUnits, additionalUnits,
  accentColor, daysElapsed, daysRemaining,
}) => {
  const currentRate = daysElapsed > 0 && currentUnits != null
    ? (currentUnits / daysElapsed).toFixed(1) : null;
  const neededRate  = daysRemaining > 0 && additionalUnits != null
    ? (additionalUnits / daysRemaining).toFixed(1) : null;
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
            ...
            {neededRate && daysRemaining > 0 && (
              <Typography variant="caption" sx={{ color: accentColor, fontWeight: 600 }}>
                Necesitas: {neededRate} uds/día
                {' '}en los próximos {daysRemaining} días
              </Typography>
            )}
            {daysRemaining === 0 && (
...
// ── ProjectionInput ───────────────────────────────────────────────────────────
const ProjectionInput = ({ sim, ebt, ing, targetUnits, onTargetChange, daysRemaining }) => {
  ...
  const projection = useMemo(() => {
    ...
    const dailyRate    = daysRemaining > 0 ? (target / daysRemaining).toFixed(1) : null;
    return {
      ebitda:    projEbitda,
  ...
  return (
    <Paper elevation={0} sx={{ p: 2.5, border: '1px solid #334155', height: '100%' }}>
      ...
      <TextField
        ...
        sx={{
          ...
          mb: 2.5, maxWidth: 240, display: 'block',
        }}
      />
        ...
        <Box>
          <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1.5, mb: 0.5 }}>
            <Typography variant="h3" fontWeight={700}
              ...
              {projection.ebitdaPct > 0 ? '+' : ''}{projection.ebitdaPct}%
            </Typography>
            <Typography variant="body2" color="text.secondary">EBITDA proyectado</Typography>
          </Box>

          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1.5 }}>
            {CLP(projection.ebitda)} · Ingreso proyectado: {CLP(projection.ingreso)}
          </Typography>
...
const ProjectionSimulator = ({
  salesLoaded, expensesLoaded, selectedMonth, isCurrentMonth, products,
}) => {
  ...
  const fetchSim = useCallback(async (prod) => {
    setLoading(true);
    setError(null);
    try {
  ...
  useEffect(() => {
    ...
    else { setSimData(null); setTargetUnits(''); }
  }, [product, fetchSim]);
  ...
  return (
    <Paper elevation={0} sx={{ p: 3, border: '1px solid #1E293B', mt: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
        ...
        <Typography variant="subtitle2">¿Cuánto Necesito Vender Este Mes?</Typography>
      </Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2.5 }}>
        <Typography variant="body2" color="text.secondary">
          Proyección para los días restantes del mes en curso.
          Llevas{' '}
          <Box component="span" sx={{ color: 'text.primary', fontWeight: 600 }}>{elapsed} días</Box>
          {' '}transcurridos y quedan{' '}
          <Box component="span" sx={{ color: 'primary.main', fontWeight: 600 }}>{remaining} días</Box>
          {' '}para cerrar el mes.
        </Typography>
      </Box>
      ...
      <Autocomplete
        options={products}
        value={product}
        onChange={(_, val) => { setProduct(val); setTargetUnits(''); setSimData(null); }}
        renderInput={(params) => (
          <TextField
            {...params}
      ...
      {loading && <Box sx={{ mt: 2 }}><CircularProgress size={24} color="primary" /></Box>}
      {error   && (
      ...
      {sim && !loading && (
        <>
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
            ...
            <Box sx={{ flex: '1 1 200px', minWidth: 0 }}>
              <TargetCard
                ...
                impossible={sim.breakeven_units === null && !sim.already_breakeven}
            ...
            <Box sx={{ flex: '1 1 200px', minWidth: 0 }}>
              <TargetCard
                icon={<TrendingUpIcon sx={{ color: '#22C55E', fontSize: 18 }} />}
          ...
          {/* Disclaimer */}
          <Box sx={{
            bgcolor: '#0F172A', border: '1px solid #334155',
            borderRadius: 2, p: 2,
          }}>
            <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
              <InfoOutlinedIcon sx={{ fontSize: 15, color: '#475569', mt: 0.3, flexShrink: 0 }} />
              <Box>
                <Typography variant="caption" fontWeight={600}
                  sx={{ color: '#94A3B8', display: 'block', mb: 0.5 }}>
                  Esta simulación es informativa y no constituye asesoría profesional
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  Los cálculos se basan en el promedio histórico del producto seleccionado durante
                  los {elapsed} días transcurridos, y asumen que el precio, el costo de
                  ingredientes y la demanda se mantienen constantes en los {remaining} días
                  restantes. En la práctica los resultados varían por cambios en el mix de ventas,
                  costos de ingredientes, variaciones de demanda, descuentos aplicados, nuevos
                  gastos operacionales o diferencias en el canal (local vs. Uber Eats). No
                  constituye asesoría financiera ni contable. Se recomienda complementar con la
                  revisión de un contador o asesor financiero antes de tomar decisiones de negocio.
                </Typography>
              </Box>
            </Box>
          </Box>
