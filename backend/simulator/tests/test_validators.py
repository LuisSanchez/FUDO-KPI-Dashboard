"""Column validation tests."""
import pandas as pd

from simulator.data_processing import validate_expenses_columns, validate_sales_columns


def test_validate_sales_columns_ok(sample_sales_df):
    ok, missing = validate_sales_columns(sample_sales_df)
    assert ok is True
    assert missing == []


def test_validate_sales_columns_missing():
    df = pd.DataFrame({"Producto": ["x"]})
    ok, missing = validate_sales_columns(df)
    assert ok is False
    assert "Id. Venta" in missing
    assert "Creación" in missing


def test_validate_expenses_columns_ok(sample_expenses_df):
    ok, missing = validate_expenses_columns(sample_expenses_df)
    assert ok is True
    assert missing == []


def test_validate_expenses_columns_missing():
    df = pd.DataFrame({"Id": [1]})
    ok, missing = validate_expenses_columns(df)
    assert ok is False
    assert "Fecha" in missing
    assert "Cancelado" in missing
